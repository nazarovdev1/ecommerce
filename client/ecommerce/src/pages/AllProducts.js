...
const AllProducts = () => {
	...
	// isLoading — yuklanayotgan bo‘lsa true bo‘ladi

	if (isLoading) {
		// Agar ma'lumot hali yuklanayotgan bo‘lsa loading ko‘rsatamiz
		return (
		...
		<div className='min-h-screen bg-gray-900'>
			...
			<div className='bg-gray-800 border-b border-gray-700'>
				<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6'>
					<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
						...
						{/* Sahifa nomi va mahsulotlar soni */}
			...
			<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
				...
				{products.length === 0 ? (
					...
					// Agar mahsulot bo‘lsa — kartalar ko‘rsatiladi
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8'>
						{products.map(product => (
							<ProductCard key={product.id} product={product} />
							// Har bir mahsulotni ProductCard componentida chiqaramiz
						))}
					</div>
				)}
			</div>
